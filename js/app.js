...
var elements = document.getElementById('duckgif');
...
function User(userName){ // This is the constructor function for our User objects.
...
Quiz.prototype.addQuestion = function(question, responseA, weightA, responseB, weightB, responseC, weightC, responseD, weightD, axis){ // A method of Quiz to build the questions a quiz object contains. Requires a question content, and the content and weight of four response options.
  this.questions.push([question, responseA, weightA, responseB, weightB, responseC, weightC, responseD, weightD, axis]);
};

Quiz.prototype.addResult = function(resultTitle, resultBio, imagePath, imageAtl, imageTitle){ // A method of Quiz to build the results of a quiz object. Our quizzes need four (4) results to work properly.
  let resultCounter = 0;
...
// Build some questions and users
...
// duckquiz.addQuestion('sample question', 'responseA', 'weightA', 'responseB', 'weightB', 'responseC', 'weightC', 'responseD', 'weightD', 'axis');
duckquiz.addQuestion('You fell asleep watching a movie you really enjoy. As you drift into your dreams, you find yourself in the best part of the story. What\'s going on in this scene?', 'Recognition for your inspirational efforts are finally happening, you\'ve started a social movement.', 'y--', 'You are experiencing peace for the first time in a long time.', 'y++', 'You are soaring above treetop canopies, fjords and volcanoes, basking in the beauty of the earth and pondering your own significance in the grand scheme.', 'x++', 'You finally figured out the villain’s motive and plan-of-action, and you’re gathering up your allies to help put a stop to it.', 'x--', -1200);
duckquiz.addQuestion('Friends invite you out to party, but your team has an assignment due tonight and you need to contribute. Do you...?', 'Quickly craft an MVP response and you’re off to Paaaaarr-TEEEHH!!', 'x++', 'Forego the fleeting distraction and apply yourself to the task at hand, for the sake of those depending upon you.', 'y++', 'Invite your friends to pre-game at your place so you can get lit and the assignment can get done. You’re so innovative and efficient, wow!', 'x--', 'What do you stand to gain from partying? You’re in your jammies right now, beverage-of-choice in hand, ready for a long night of procrastinating this assignment.', 'y--', -1050); // Add a question to 'duckquiz' Quiz
duckquiz.addQuestion('You’ve been tasked with creating a dynamic, interactive app! Any app you want! Your mind is an array of ideas, and you can’t wait to access them all! Which bit of coding are you most looking forward to?', 'JavaScript! Logic is your thing. You excel at creating the moving parts and making sure they work together harmoniously. ', 'x--', 'HTML/CSS. You love creating shiny, beautiful interfaces for the world to enjoy!', 'x++','Compiling ideas and inspiration. Basically everything that leads up to the actual work.', 'y++', 'Getting a group together to help you achieve your goals! You thrive when you partner with others, and you’re excited to bond over some code.', 'y--', -900);
...
duckquiz.addQuestion('You are fortunate enough to enjoy one whole day off of school/work each week. The past six days have been especially silly, and you can’t wait to be off duty. What are you doing?', 'You’re inside! There’s nothing like the comfort of your home after a long week. Netflix, a good book, a home-cooked meal, your stack of unfinished projects... The possibilities are endless.', 'x++', 'You’re outside! You’re energized by the bustle of the world and like to get out whenever you can. Plus, people watching is the best on Sundays.', 'x--', 'You’re gonna sleep in and take it slow for the morning, but you might pop out for brunch. Or lunch. Or dinner. Or drinks. Or all of those.', 'y++', 'A day off? Sounds like the perfect opportunity for some quality time with your crew. The homies have missed you.', 'y--', -600);
duckquiz.addQuestion('Your ideal workspace is...', 'Wherever the work takes you, my dude', 'x++', 'A brightly lit space, canvas and art materials abound, with a garden you can paint self-portraits in.', 'y--', 'A home office, well, actually more of a home library. You’d have a comfy chair that’s good for pondering and a lamp that casts just the right amount of light.', 'y++', 'Anywhere active and exciting!', 'x--', -450);
duckquiz.addQuestion('What’s your dream job?', 'Artist who is compensated appropriately for their work', 'y--', 'Detective or Analyst of some sort', 'y++', 'Warrior with a heart of gold', 'x--', 'Stay-at-home parent / house-spouse', 'x++', -300);
duckquiz.addQuestion('What\'s on your mind lately?', 'So much. You can barely keep it all contained.', 'x--', 'Why is the world so bad?', 'x++', 'Honestly…? Not much.', 'y++', 'You’re preoccupied, thinking about adventures and such.', 'y--', -150);
...
// Helper functions
function mapXY(){ // This function takes in our x and y values, and converts them into an index value (0-3), that we call 'quadrant' because we use a graph to visualize the relation between x/y and the results.
...
function optionEvents(element, option){ // This function contains an event listener we use for the options. It will call newResponse according to option, move currentQuestion to the next value, and depending if there are remaining questions, call either renderQuestion (for the next question now), or call lastResponse.
  element.addEventListener('click', function(event){
    ...
    // Add the 'move-left' id if it doesn't exist, to move duck gif
    if (!document.getElementById('move-left')){
      elements.setAttribute('id', 'move-left');
    }

    newResponse(quizObjects[0].questions[currentQuestion][option]);
    // Creates an array of the current questions
    var curQArr = quizObjects[0].questions[currentQuestion];
    // References the last item in the current question array, the # pixels to decrement its position on the x axis
    var curAxis = curQArr[curQArr.length - 1];
...
function renderQuestion() { // This function renders our main feature to the page with its content, and adds event listeners to the options (only when the first question is loaded).
  ...
  output2.innerText = quizObjects[0].questions[currentQuestion][5];
  output3.innerText = quizObjects[0].questions[currentQuestion][7];
...
// Hide welcome instructions upon username submit, and begin showing questions
